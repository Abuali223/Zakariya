...
if sudo docker ps --format '{{.Names}}' 2>/dev/null | grep -qx "$DB_CONT"; then
  for f in audit-2.sql audit-3.sql audit-4.sql audit-5.sql audit-6.sql bus.sql audit-7.sql reattribute-avans.sql audit-8.sql pay-notes.sql reverse-payment.sql; do
    echo "    -> $f"
