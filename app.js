...
async function srsUpdate(user,item,grade){
  ...
  await dbPut(rec); enqueue([{op:'upsert_progress',row:rec}]);
  alert(grade>=3?'Ajoyib!':'Qiyin belgilandi');
}

/* 4) Queue + (ixtiyoriy) Supabase sync */
const QKEY='sync_q';
function enqueue(rows){const q=JSON.parse(localStorage.getItem(QKEY)||'[]'); q.push(...rows); localStorage.setItem(QKEY,JSON.stringify(q));}
...
$('#btn-sync')?.addEventListener('click',flush); setInterval(flush,10000); addEventListener('online',flush);

/* 5) Pages */
function dash(){
  const total=Object.values(CONTENT.packs).reduce((n,p)=>n+p.items.length,0);
  $('#page').innerHTML=`<h2>Dashboard</h2><div class="progress"><div id="gprog" style="width:0%"></div></div><p>Jami element: ${total}</p>`;
}
function lessons(){
  const cards=Object.entries(CONTENT.packs).map(([k,p])=>`
    <div class="card"><b>${p.title}</b><div>${p.items.length} so‘z</div>
    <button class="btn" onclick="openPack('${k}')">Ko‘rish</button></div>`).join('');
...
window.markHard=i=>srsUpdate(currentUser,i,2); window.markGood=i=>srsUpdate(currentUser,i,5);
...
function tests(){ $('#page').innerHTML=`<h2>Testlar</h2><div class="card">Demo test: tez orada.</div>`; }
...
function profile(){ $('#page').innerHTML=`<h2>Profil</h2><div class="card">Login yoqsangiz, progress qurilmalar o‘rtasida sinxron bo‘ladi.</div>`; }
...
/* 8) Init */
(async()=>{ await initSupabase(); if(!location.hash) location.hash='#dash'; nav(); })();